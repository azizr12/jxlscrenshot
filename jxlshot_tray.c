...
static void show_tray_menu(HWND hwnd) {
    ...
    if (g_hwndMenuOwner) ApplyDarkMode(g_hwndMenuOwner);
    if (g_hwndMenuOwner) SetForegroundWindow(g_hwndMenuOwner);
...
static void execute_full_capture(void) {
    GrabDXGI g_dxgi;
    ZeroMemory(&g_dxgi, sizeof(g_dxgi));
    ...
    if (!save_dxgi_as_jxl(g_dxgi.bits, g_dxgi.w, g_dxgi.h, g_dxgi.format, g_cfg.lossless, g_cfg.distance, g_cfg.force_sdr, out_path)) {
        MessageBoxW(NULL, L"Encoding or saving failed.", L"jxlshot", MB_ICONERROR);
    }
...
static void crop_and_encode_region(RECT *r) {
    Grab g;
    ZeroMemory(&g, sizeof(g));
    if (!grab_primary_monitor(&g)) return;
    ...
    switch (msg) {
        case WM_PAINT: {
            ...
            if (g_isDragging || (g_rcSel.right > g_rcSel.left && g_rcSel.bottom > g_rcSel.top)) {
                // Ternary operators used to completely avoid C++ std::min/max vs Windows macro conflicts
                int x = (g_rcSel.left < g_rcSel.right) ? g_rcSel.left : g_rcSel.right;
                int y = (g_rcSel.top < g_rcSel.bottom) ? g_rcSel.top : g_rcSel.bottom;
                int w = (g_rcSel.right > g_rcSel.left) ? (g_rcSel.right - g_rcSel.left) : (g_rcSel.left - g_rcSel.right);
                int h = (g_rcSel.bottom > g_rcSel.top) ? (g_rcSel.bottom - g_rcSel.top) : (g_rcSel.top - g_rcSel.bottom);
...
static void start_region_capture(void) {
    ...
    g_hwndRegion = CreateWindowExW(WS_EX_TOPMOST | WS_EX_TOOLWINDOW, L"JxlShotRegionClass", L"", WS_POPUP, 0, 0, g_screenW, g_screenH, NULL, NULL, GetModuleHandle(NULL), NULL);
...
static BOOL check_modifiers(UINT required_mod) {
    ...
    if (ctrl_pressed != ((required_mod & MOD_CONTROL) != 0)) return FALSE;
    if (shift_pressed != ((required_mod & MOD_SHIFT) != 0)) return FALSE;
    if (alt_pressed != ((required_mod & MOD_ALT) != 0)) return FALSE;
    if (win_pressed != ((required_mod & MOD_WIN) != 0)) return FALSE;
    return TRUE;
}
