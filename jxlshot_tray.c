...
// Forward declarations for UI components (defined in custom_theme.c)
extern void init_custom_ui(HINSTANCE hInst);
extern void show_tray_menu(HWND hwnd);
extern void execute_about(void);

// Core state (non-static so custom_theme.c can access g_hwndTray)
...
static void execute_check_update(HWND hwnd) {
    ...
    for (int i = (int)wcslen(remote_version_str) - 1; i >= 0; i--) {
...
LRESULT CALLBACK RegionWndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    switch (msg) {
        ...
        case WM_DESTROY: {
            ...
            g_regionCaptureEndTime = GetTickCount();
...
// Include the separated UI components
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrev, LPSTR szCmdLine, int sw) {
    ...
    // Initialize custom UI (registers menu window class)
    init_custom_ui(hInst);
