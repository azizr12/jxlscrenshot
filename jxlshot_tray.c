...
typedef enum _PreferredAppMode { Default = 0, AllowDark = 1, ForceDark = 2, ForceLight = 3, Max = 4 } PreferredAppMode;
...
static void ApplyDarkMode(HWND hwnd) {
    ...
    if (hUxtheme) {
        fnSetPreferredAppMode pSet = (fnSetPreferredAppMode)GetProcAddress(hUxtheme, MAKEINTRESOURCEA(135));
        fnAllowDarkModeForWindow pAllow = (fnAllowDarkModeForWindow)GetProcAddress(hUxtheme, MAKEINTRESOURCEA(133));
        fnFlushMenuThemes pFlush = (fnFlushMenuThemes)GetProcAddress(hUxtheme, MAKEINTRESOURCEA(136));
        if (pSet && pAllow && pFlush) { pSet(AllowDark); pAllow(hwnd, TRUE); pFlush(); }
...
static void show_tray_menu(HWND hwnd) {
    ...
    if (g_hwndMenuOwner) { ApplyDarkMode(g_hwndMenuOwner); SetForegroundWindow(g_hwndMenuOwner); }
...
static void execute_about(void) {
    ...
    config.pszContent = L"Minimal tray screenshot tool using JPEG XL.\n\n<a href=\"https://github.com/azizr12/jxlscrenshot\">https://github.com/azizr12/jxlscrenshot</a>";
    ...
    switch (msg) {
        case WM_PAINT: {
            ...
            if (g_isDragging || (g_rcSel.right > g_rcSel.left && g_rcSel.bottom > g_rcSel.top)) {
                ...
                wchar_t dimText[64]; _snwprintf(dimText, 64, L"%d × %d", w, h);
        ...
        case WM_MOUSEMOVE:
            if (g_isDragging) { g_rcSel.right = GET_X_LPARAM(lp); g_rcSel.bottom = GET_Y_LPARAM(lp); InvalidateRect(hwnd, NULL, FALSE); }
            return 0;
        case WM_LBUTTONUP:
            if (g_isDragging) {
        ...
        case WM_KEYDOWN:
            if (wp == VK_ESCAPE) {
                g_isDragging = FALSE; ReleaseCapture(); ShowWindow(hwnd, SW_HIDE); DestroyWindow(hwnd);
                RedrawWindow(NULL, NULL, NULL, RDW_INVALIDATE | RDW_UPDATENOW | RDW_ALLCHILDREN | RDW_ERASE);
            }
            return 0;
        case WM_DESTROY:
            ...
            g_hwndRegion = NULL; g_isDragging = FALSE;
...
static void start_region_capture(void) {
    ...
    g_isDragging = FALSE; g_rcSel.left = g_rcSel.top = g_rcSel.right = g_rcSel.bottom = 0;
    ...
    RECT rc = {0, 0, 1, 1}; FillRect(g_hdcBlack, &rc, (HBRUSH)GetStockObject(BLACK_BRUSH));
...
static BOOL check_modifiers(UINT required_mod) {
    ...
    if (ctrl != ((required_mod & MOD_CONTROL) != 0)) return FALSE;
    if (shift != ((required_mod & MOD_SHIFT) != 0)) return FALSE;
    if (alt != ((required_mod & MOD_ALT) != 0)) return FALSE;
    if (win != ((required_mod & MOD_WIN) != 0)) return FALSE;
    return TRUE;
}
...
static LRESULT CALLBACK LowLevelKeyboardProc(int nCode, WPARAM wParam, LPARAM lParam) {
    if (nCode == HC_ACTION && wParam == WM_KEYDOWN) {
        ...
        if (g_cfg.hk_full_vk != 0 && pKB->vkCode == g_cfg.hk_full_vk && check_modifiers(g_cfg.hk_full_mod)) {
            dbg("MATCH: Triggering FULL capture");
            PostMessageW(g_hwndTray, WM_HOOK_FULL_CAPTURE, 0, 0);
            return 1;
        }
        if (g_cfg.hk_region_vk != 0 && pKB->vkCode == g_cfg.hk_region_vk && check_modifiers(g_cfg.hk_region_mod)) {
...
LRESULT CALLBACK TrayWndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    switch (msg) {
        case WM_TRAYICON:
            if (lp == WM_RBUTTONUP || lp == WM_LBUTTONUP) show_tray_menu(hwnd);
            break;
        ...
        case WM_COMMAND:
            switch (LOWORD(wp)) {
                ...
                case IDM_RELOAD: init_config(); break;
        ...
        case WM_DESTROY:
            ...
            if (g_hwndMenuOwner) { DestroyWindow(g_hwndMenuOwner); g_hwndMenuOwner = NULL; }
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrev, LPSTR szCmdLine, int sw) {
    set_dpi_aware(); init_paths(); ensure_default_ini(); init_config(); dbg_init();
    WNDCLASSEXW wc = {0};
    wc.cbSize = sizeof(wc); wc.lpfnWndProc = TrayWndProc; wc.hInstance = hInst;
    ...
    g_hwndMenuOwner = CreateWindowExW(WS_EX_TOOLWINDOW, L"JxlShotTrayClass", L"", WS_POPUP, 0, 0, 0, 0, NULL, NULL, hInst, NULL);
    ShowWindow(g_hwndMenuOwner, SW_HIDE); ApplyDarkMode(g_hwndMenuOwner);
